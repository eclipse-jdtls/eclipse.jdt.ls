...
public class ChangeUtil {
	...
	public static void convertResourceChange(ResourceChange resourceChange, WorkspaceEdit edit) throws CoreException {
		...
		} else if (resourceChange instanceof CreatePackageChange) {
			convertCreatePackageChange(edit, (CreatePackageChange) resourceChange);
		} else if (resourceChange instanceof MoveCompilationUnitChange) {
			convertMoveCUResourceChange(edit, (MoveCompilationUnitChange) resourceChange);
		}
	}
	...
	private static void convertCreatePackageChange(WorkspaceEdit edit, CreatePackageChange packageChange) throws CoreException {
		IPackageFragment pack = (IPackageFragment) packageChange.getModifiedElement();
		IPath newPackagePath = pack.getResource().getLocation();
	...
	private static void convertMoveCUResourceChange(WorkspaceEdit edit, MoveCompilationUnitChange cuChange) {
		ICompilationUnit modifiedCU = (ICompilationUnit) cuChange.getModifiedElement();
		RenameFile rf = new RenameFile();

		IPath prefix = modifiedCU.getParent().getResource().getLocation().removeLastSegments(cuChange.getOldPackage().getPath().segmentCount());
		IPath newPath = prefix.append(cuChange.getDestinationPackage().getPath());
		newPath = newPath.append(modifiedCU.getResource().getLocation().lastSegment());
