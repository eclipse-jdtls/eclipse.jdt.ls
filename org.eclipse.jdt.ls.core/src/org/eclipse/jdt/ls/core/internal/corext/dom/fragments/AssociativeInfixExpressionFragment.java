 ...
 *
 * Copied from org.eclipse.jdt.internal.corext.dom.fragments.AssociativeInfixExpressionFragment
 *
 * Contributors:
...
class AssociativeInfixExpressionFragment extends ASTFragment implements IExpressionFragment {
	...
	/**
	 * Returns all matching subsequences of <code>toMatch</code> in
	 * <code>source</code>.
	 *
	 * @param source
	 *            the source to look for matching subsequences
	 * @param toMatch
	...
	private static boolean matchesAt(int index, List<Expression> subject, List<Expression> toMatch) {
		...
		for (int i = 0; i < toMatch.size(); i++, index++) {
			if (!JdtASTMatcher.doNodesMatch(subject.get(index), toMatch.get(i))) {
				return false;
			}
		}
		return true;
	...
	private IASTFragment[] getSubFragmentsWithMyNodeMatching(IASTFragment toMatch) {
		...
		List<List<Expression>> matchingSubsequences = getMatchingContiguousNodeSubsequences(getOperands(), kinToMatch.getOperands());
