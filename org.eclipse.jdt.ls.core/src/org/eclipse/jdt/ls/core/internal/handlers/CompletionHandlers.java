...
public final class CompletionHandlers extends ExtensionRepository<ICompletionHandler> {
	...
	public CompletionHandlers(PreferenceManager manager) {
		super(EXTENSION_POINT_ID, ICompletionHandler.class);
		this.manager = manager;
		CompletionHandler defaultHandler = new CompletionHandler(manager);
		setDefaultImplementers(Arrays.asList(defaultHandler));
	}
	...
	public Either<List<CompletionItem>, CompletionList> completion(CompletionParams params, IProgressMonitor monitor) {
		...
		boolean defaultsSupport = manager.getClientPreferences().isCompletionListItemDefaultsSupport();
		for (int i = 0; i < handlers.size(); ++i) {
			...
			if (c != null) {
				...
				if (defaults == null && defaultsSupport) {
