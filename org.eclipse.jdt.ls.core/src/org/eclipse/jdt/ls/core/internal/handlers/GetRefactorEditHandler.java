...
public class GetRefactorEditHandler {
	...
	public static RefactorWorkspaceEdit getEditsForRefactor(GetRefactorEditParams params) {
		...
		try {
			...
			} else if (InvertBooleanSubProcessor.INVERT_VARIABLE_COMMAND.equals(params.command)) {
				proposal = (LinkedCorrectionProposal) InvertBooleanSubProcessor.getInvertVariableProposal(params.context, context, context.getCoveringNode(), false);
