 ...
 *
 * Copied from org.eclipse.jdt.internal.corext.dom.fragments.ASTFragmentFactory
 *
 * Contributors:
...
public class ASTFragmentFactory {
	...
	private static boolean rangeIncludesNonWhitespaceOutsideNode(ISourceRange range, ASTNode node, ICompilationUnit cu) throws JavaModelException {
		return Util.rangeIncludesNonWhitespaceOutsideRange(range, new SourceRange(node.getStartPosition(), node.getLength()), cu.getBuffer());
	}
