 ...
 *
 * Originally copied from org.eclipse.jdt.internal.corext.fix.PotentialProgrammingProblemsFix
 *
 * Contributors:
				...
				for (int i = 0; i < bindings.length; i++) {
					...
					if (curr instanceof ITypeBinding) {
						...
						try {
							...
							result.addWarning(
									Messages.format(FixMessages.PotentialProgrammingProblemsFix_calculatingUIDFailed_exception, new String[] { BasicElementLabels.getJavaElementName(typeBinding.getName()), e.getLocalizedMessage() }),
									JavaStatusContext.create(types[i]));
