...
def main(args):
	...
	if platform.system() == 'Windows' and 'APPDATA' in os.environ:
		cachedir = Path(os.environ['APPDATA']) / 'jdtls'
	elif platform.system() == 'Darwin' and 'HOME' in os.environ:
		cachedir = Path(os.environ['HOME']) / 'Library' / 'Caches' / 'jdtls'
	elif platform.system() == 'Linux' and 'HOME' in os.environ:
		cachedir = Path(os.environ['HOME']) / '.cache' / 'jdtls'
	else:
		cachedir = tempfile.gettempdir() / 'jdtls'
